...
Private **version 15** succeeded on **24 September 2026 at 18:41:38 UTC**, from Site source `bc5ee85d0a5a096e582f055f4e17b71636348160`. The saved version is `appgprj_6a9bdfc1819481918c7085729f869ca9~appgver_53e0db02f4c481919b61c52345fba5fe`; deployment `appgdep_6ab56ed235d4819182a4273ce5a0762d` reports `succeeded`. Owner-only access was preserved.

This publication adds the Baden train-exit and provider-recovery fixes, including validated OSRM bicycle fallback. It retains the three comparisons, per-leg prerequisites, protected OJP backend and scoped TripInfo checks. Environment revision remains **0**: live OJP website access is pending the separate Site runtime secret. GitHub Actions API access is independently verified.

129 application tests and the frontend/Worker production builds passed. The unchanged Python evaluation suite was last verified with 13 passing tests. All three reported route pairs returned useful results in live checks for 25 September at 08:00 Europe/Zurich; see [the experiment log](EXPERIMENTS.md). The managed preview service was unavailable, so new desktop/mobile browser QA could not be completed. Older `SOURCE.md` and `project-documentation` files in Site history are dated publication snapshots; current documentation is authoritative here in GitHub.
