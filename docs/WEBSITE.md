...
Private **version 17** succeeded on **24 September 2026 at 20:43:17 UTC**, from Site source `3ecbd8f6745d56d1ce4296f81c5618f168300348`. The saved version is `appgprj_6a9bdfc1819481918c7085729f869ca9~appgver_93baa3fd33a88191a4abd0c3f23d3732`; deployment `appgdep_6ab58b4fe8308191a0abf2888aeec6ec` reports `succeeded`. Owner-only access was preserved.

This publication separates verified access from ticket/reservation uncertainty, labels actual journey permission, applies reviewed ordinary ZVV-operator/tpg bus/tram rules, and adds adjustable Relaxed/Regular/Strong/Electric cycling profiles with slope-dependent timing and profile-aware caching. The earlier all-mode selector, public service symbols, SBB IR/SOB defaults and overnight rail-exit fixes remain. The prior Baden/provider-recovery fixes remain. Environment revision remains **0**: OJP runtime activation is pending, while keyless public bicycle details are active.
...
Older `SOURCE.md` and `project-documentation` files in Site history are dated snapshots. Current project documentation is authoritative here in GitHub. App runtime, configuration and test files match this exact deployed Site source; the GitHub app README is maintained with current project documentation. A GitHub push alone does not publish a Site version.
