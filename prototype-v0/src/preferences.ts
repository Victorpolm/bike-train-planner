...
export function preferenceOptions(cycling: CyclingPreference, endpointPreference: EndpointPreference, busPreference: BusPreference = "include-unknown", bicycleScope?: BicycleScope, cyclingPace: CyclingPace = DEFAULT_CYCLING_PACE): Options {
  const budgets = {
  ...
  return { ...DEFAULT_OPTIONS, ...budgets[cycling], cyclingPace: { ...cyclingPace }, endpointPreference, busPreference, ...(bicycleScope ? { bicycleScope } : {}) };
}
