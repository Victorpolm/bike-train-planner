...
**Buses with my bicycle** defaults to operators with a matched published bicycle policy, subject to their conditions. You can also include unverified buses or avoid buses. Matched bicycle prohibitions are always excluded. Bus cards, travel-plan legs and map pins show the status; expanded plans link to the official rules and explain boarding, tickets and reservations. The guidance covers a standard, unfolded bicycle. Individual departures, reservation availability and live bike spaces are not confirmed. See [bus bicycle coverage and limits](https://github.com/Victorpolm/bike-train-planner/blob/main/docs/BUS_BICYCLES.md).
